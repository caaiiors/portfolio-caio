...
const CARD_SELECTORS = '.glass-surface, [data-no-particles]';
...
const Particles = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = e => {
      const overCard = e.target.closest?.(CARD_SELECTORS);
      if (overCard) {
    ...
    const update = t => {
      ...
      if (moveParticlesOnHover) {
        mouseSmoothRef.current.x += (mouseTargetRef.current.x - mouseSmoothRef.current.x) * LERP_SPEED;
        mouseSmoothRef.current.y += (mouseTargetRef.current.y - mouseSmoothRef.current.y) * LERP_SPEED;
        particles.position.x = -mouseSmoothRef.current.x * particleHoverFactor;
        particles.position.y = -mouseSmoothRef.current.y * particleHoverFactor;
      }
      if (!disableRotation) {
